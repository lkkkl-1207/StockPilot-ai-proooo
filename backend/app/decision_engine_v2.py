...
def _download_from_alpha_vantage(symbol: str) -> pd.DataFrame:
    """从 Alpha Vantage 获取最近约 100 个交易日的日线数据。"""
...
def _download_from_yfinance(symbol: str) -> pd.DataFrame:
    """Alpha Vantage 和 Finnhub 不可用时，用 yfinance 作为最后备用。"""
...
def download_price_history(
    ...
    if data.empty:
        raise ValueError("三个历史数据源都暂时不可用，请稍后重试。")
    ...
    if len(data) < 80:
        raise ValueError("历史数据不足 80 个交易日，暂时无法计算可靠区间。")
...
def _build_rating(
    ...
    if risk_reward_ratio >= 2 and technical_score >= 70:
        return "重点关注", "技术结构和风险收益比较理想，可考虑分批观察。"

    if risk_reward_ratio >= 1.5 and technical_score >= 60:
        return "可以观察", "风险收益比较好，可等待合适位置逐步进入。"

    if risk_reward_ratio >= 1:
        return "耐心等待", "风险收益比一般，建议等待更好的价格。"

    return "暂不建议入场", "当前风险收益比不足，建议继续观察。"
...
def build_decision(symbol: str) -> dict[str, Any]:
    ...
    data["BBUpper"] = data["BBMiddle"] + 2 * rolling_std
    data["BBLower"] = data["BBMiddle"] - 2 * rolling_std
    ...
    if current_price is None or atr14 is None:
        raise ValueError("无法计算最新价格或 ATR。")
    ...
    if ma20 is not None:
        if current_price >= ma20:
            score += 8
            reasons.append("当前价格位于 20 日均线上方")
        else:
            score -= 8
            risks.append("当前价格位于 20 日均线下方")

    if ma20 is not None and ma50 is not None:
        if ma20 >= ma50:
            score += 12
            reasons.append("20 日均线高于 50 日均线，中期趋势偏强")
        else:
            score -= 12
            risks.append("20 日均线低于 50 日均线，中期趋势偏弱")

    if ma50 is not None and ma200 is not None:
        if ma50 >= ma200:
            score += 10
            reasons.append("50 日均线高于 200 日均线，长期结构偏多")
        else:
            score -= 10
            risks.append("50 日均线低于 200 日均线，长期结构偏弱")

    if rsi14 is not None:
        if 40 <= rsi14 <= 60:
            score += 10
            reasons.append("RSI 处于相对健康的中性区间")
        elif 60 < rsi14 <= 70:
            score += 4
            reasons.append("RSI 偏强，但尚未进入明显超买区")
        elif rsi14 > 70:
            score -= 12
            risks.append("RSI 超过 70，短线追高风险较大")
        elif rsi14 < 30:
            score += 3
            risks.append("RSI 低于 30，可能超卖，但仍需确认趋势")
    ...
    if macd is not None and macd_signal is not None:
        if macd >= macd_signal:
            ...
            score -= 8
            risks.append("MACD 位于信号线下方")

    if bollinger_upper is not None and current_price >= bollinger_upper:
        score -= 8
    ...
    nearest_support = (
        max(valid_supports)
        if valid_supports
        else current_price - atr14
    )

    buy_zone_low = max(0.01, nearest_support - atr14 * 0.45)
    buy_zone_high = min(current_price, nearest_support + atr14 * 0.30)

    if buy_zone_low > buy_zone_high:
        buy_zone_low = max(0.01, current_price - atr14)
    ...
    third_target = max(
        second_target + atr14,
        current_price + atr14 * 3.5,
    )

    entry_reference = (buy_zone_low + buy_zone_high) / 2
    ...
    return {
        ...
        "suggested_plan": {
            ...
            "reference_entry": round(entry_reference, 2),
            "risk_reference": round(risk_reference, 2),
            "first_take_profit": round(first_target, 2),
            "second_take_profit": round(second_target, 2),
            "third_take_profit": round(third_target, 2),
        ...
        "indicators": {
            ...
            "bollinger_upper": _round_or_none(bollinger_upper),
            "bollinger_lower": _round_or_none(bollinger_lower),
            "support_20d": round(support_20, 2),
            "resistance_20d": round(resistance_20, 2),
            "support_60d": round(support_60, 2),
            "resistance_60d": round(resistance_60, 2),
        },
